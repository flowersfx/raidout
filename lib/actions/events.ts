...
export async function getEvents() {
  ...
  return prisma.event.findMany({
    ...
    include: {
      ...
      artists: { select: { intakeUpdatedAt: true } },
    },
  });
}

export async function markArtistsTabViewed(eventId: string) {
  await prisma.event.update({
    where: { id: eventId },
    data: { artistsLastReviewedAt: new Date() },
  });
  revalidatePath("/");
}
    ...
    for (const a of source.artists) {
      await tx.artist.create({
        data: {
          ...
          // intakeToken: new unique token auto-generated by Prisma default
          // intakeUpdatedAt: left null (fresh copy starts with no intake submission)
        },
      });
    }
    ...
    for (const a of data.artists) {
      await tx.artist.create({
        data: {
          ...
          soundcheckStart: a.soundcheckStart,
          soundcheckEnd: a.soundcheckEnd,
          soundcheckMinLength: a.soundcheckMinLength ?? null,
          sortOrder: a.sortOrder,
        },
      });
    }
